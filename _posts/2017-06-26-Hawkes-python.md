...
1. **Maximum-likelihood estimation (MLE).**  We can actually express, in closed-form, the (log) likelihood of the data under a choice of parameters.  As a result, we can simply do something like $$\textrm{min} -\mathcal{L}(\Theta)$$ for log-likelihood function $$\mathcal{L}$$, applying whatever optimization machinery we choose.  However, in practice the log-likelihood is extremely flat near the optimum, leading to slow convergence, degenerate Hessians, etc.  (See figure below.)
